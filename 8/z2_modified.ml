...
*)
module GraphFunctor (Vert:VERTEX) (Edg:EDGE with type v = Vert.t and type label = (Vert.label * Vert.label)) : GRAPH with module V = Vert and module E = Edg and type vertex = Vert.t and type edge = Edg.t =
struct
	module V = Vert
	type vertex = V.t
	module E = Edg
...
*)
module Traveller (Cont:CONTAINER) (Gr:GRAPH) =
struct
	(* start must be a vertex belonging to graph *)
	let traverse graph start =
		let rec bfsaux queue acc =
			try 
				let v = Cont.peek queue
				in let nqueue = Cont.pop queue
				in let chldrn = Gr.succ graph v
				in let nonvischldrn = List.filter 
					(fun x -> not (List.mem x acc)) chldrn
				in let nacc = if List.mem v acc then acc else v::acc
				in bfsaux (Cont.multipush nonvischldrn nqueue) nacc
			with Cont.EmptyContainer -> List.rev_map (fun v -> Gr.V.label v) acc
		in bfsaux Cont.(push start empty) [];;
end;;
...
(*let bfsgr = bfs gr v1;;
let dfsgr = dfs gr v1;;*)
module Bfser = Traveller (Queue);;
module BfsG = Bfser (Graph);;
let bfst = BfsG.traverse gr v1;;
module Dfser = Traveller (Stack);;
module DfsG = Dfser (Graph);;
let bfst = DfsG.traverse gr v1;;
